...
function buildSignals(item, signature) {
  ...
  return signals.filter((signal) => signal.points > 0);
}
...
module.exports = {
  id: 'startup-persistence-scan',
  name: 'Startup Persistence Scanner',
  description: 'Inspect Run keys, startup folders, scheduled tasks, and services for risky persistence.',
  category: 'Security',
  icon: 'list-checks',
  version: '1.0.0',
  permissions: ['registry-read', 'scheduled-tasks-read', 'services-read', 'file-signature-read'],
  run: async (args, ctx) => {
    const rawItems = [
      ...(await getRegistryRunItems()),
      ...(await getStartupFolders()),
    ...
    for (const item of deduped.slice(0, limit)) {
      enriched.push(await enrichStartupItem(item));
    }

    enriched.sort((a, b) => b.risk.score - a.risk.score || String(a.name).localeCompare(String(b.name)));
    const summary = {
      total: enriched.length,
      registry: enriched.filter((item) => item.source === 'Registry Run').length,
      startupFolders: enriched.filter((item) => item.source === 'Startup Folder').length,
      scheduledTasks: enriched.filter((item) => item.source === 'Scheduled Task').length,
      services: enriched.filter((item) => item.source === 'Windows Service').length,
      risky: enriched.filter((item) => item.risk.score >= 35).length,
      highRisk: enriched.filter((item) => item.risk.score >= 60).length
    };

    if (ctx.appStore) {
      ctx.appStore.addHistory('startup', { summary }, 20);
    }
